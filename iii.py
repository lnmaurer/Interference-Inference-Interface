...
nStable = int(
    (barrierX + sqrt((canvasX - barrierX)**2 + canvasY**2)) * d / c / dt)
...
def redrawBarrierFrame():
    ...
    spinboxes = [
    ]  # spinboxes for entering the top and bottom positions of the openings
    intVars = []  # need to save StringVars or else they get garbage collected
    distStrVars = [
    ]  # stringVars that report the distance from the center of the gaps to the slice intersection
    r = 3  #rows 0,1,2 already taken by widgets for adding an opeing

    for gap in gaps:
        oNum = r - 3  #opening number
        frame = tkinter.ttk.Labelframe(barrierFrame,
                                       text="Opening {}".format(oNum))
        frame.grid(column=0,
                   row=r,
                   columnspan=2,
                   sticky='nesw',
        ...
        tkinter.ttk.Label(frame, text="Bottom:").grid(column=0,
                                                      row=0,
                                                      sticky='nes',
                                                      padx=5,
                                                      pady=5)
        ...
        tkinter.ttk.Label(frame, text="Top:").grid(column=0,
                                                   row=1,
                                                   sticky='nes',
                                                   padx=5,
                                                   pady=5)
        ...
        tkinter.ttk.Label(frame, textvariable=distStrVar).grid(column=0,
                                                               row=2,
                                                               sticky='nes',
                                                               padx=5,
                                                               pady=5)
        ...
        tkinter.ttk.Button(frame,
                           text='Remove',
                           command=lambda n=oNum: removeOpening(n)).grid(
                               column=0,
                               row=4,
                               sticky='nsew',
                               columnspan=2,
...
def spinboxChanged(openingNumber):
    ...
    try:
        ...
        if (
            (gaps[openingNumber][1] != currentTop) or
            (gaps[openingNumber][0] != currentBot)
        ) and goodOpeningBot(openingNumber) and goodOpeningTop(
                openingNumber
        ):  #if either the top or bottom spibox value is different from the stored values and the positions are good, enable the update button
...
def goodOpeningTop(openingNumber):
    ...
    if ((openingNumber == (len(gaps) - 1)) and (value < canvasY) and
        (value > bottom)) or ((openingNumber < (len(gaps) - 1)) and
                              (value < gaps[openingNumber + 1][0]) and
                              (value > bottom)):
        return True
    else:
        return False
...
def goodOpeningBot(openingNumber):
    ...
    if ((openingNumber == 0) and (value > 0) and
        (value < top)) or ((openingNumber > 0) and
                           (value > gaps[openingNumber - 1][1]) and
                           (value < top)):
        return True
    else:
        return False
...
def updateOpening(openingNumber):
    ...
    try:
        ...
        if (
            (gaps[openingNumber][1] != top) or (gaps[openingNumber][0] != bot)
        ) and goodOpeningTop(openingNumber) and goodOpeningBot(
                openingNumber
        ):  #don't do anything if nothing has changed or if the opening is bad
    ...
    spinboxChanged(
        openingNumber
    )  #regaurdless of what happened, the update box should be disabled
...
def updateEzPlot():
    ...
    data = (transpose(EzVis[:, :]) / maxEz +
            1) / 2 * 256  #+1 so that zero is in the center
...
def updateEzRMSPlot():
    ...
    im = Image.fromarray(data)
    ezRMSPlot = ImageTk.PhotoImage(
        image=im)  # assign to avoid garbage collection
...
def makeAvgedTrace(xS, yS, invert=False, othercoord=None):
  ...
  and the y values are similiarly stored like [V0,...,Vn,-Vn,...,-V0], so that we plot +/- of the value"""
    if avgSetting.get(
    ) == 'amp':  #want to display amplitude = EzRMS*sqrt(2) with zero in middle of plot
...
def redrawCanvases():
    ...
    for i in range(0, len(invGaps), 2):
        Ezcanvas.create_line([(barrierX, invGaps[i]),
                              (barrierX, invGaps[i + 1])],
                             width=1,
                             fill='red')
        EzRMScanvas.create_line([(barrierX, invGaps[i]),
                                 (barrierX, invGaps[i + 1])],
                                width=1,
                                fill='red')
    ...
    if distLineSetting.get() == "lines":
        for gap, distStrVar in zip(gaps, distStrVars):
            #draw the lines
            yGap = (gap[0] + gap[1]) / 2
            Ezcanvas.create_line([(barrierX, yGap), (sliceX, sliceY)],
                                 width=1,
                                 fill='orange',
                                 dash=',')
            EzRMScanvas.create_line([(barrierX, yGap), (sliceX, sliceY)],
                                    width=1,
                                    fill='orange',
                                    dash=',')
            ...
            Ezcanvas.create_text(xText,
                                 yText,
                                 text=distStrVar.get(),
                                 fill="Cyan",
                                 font=("Helvetica", "12"))
            EzRMScanvas.create_text(xText,
                                    yText,
                                    text=distStrVar.get(),
                                    fill="Cyan",
                                    font=("Helvetica", "12"))
    ...
    lineID = Ezcanvas.create_line([(0, sliceY), (canvasX, sliceY)],
                                  width=1,
                                  fill='yellow',
                                  dash='-')
    Ezcanvas.tag_bind(lineID, "<Button-1>", horizClickMethod)
    lineID = EzRMScanvas.create_line([(0, sliceY), (canvasX, sliceY)],
                                     width=1,
                                     fill='green',
                                     dash='-')
    EzRMScanvas.tag_bind(lineID, "<Button-1>", horizClickMethod)
    lineID = VertPlotCanvas1.create_line([(0, sliceY), (plotD, sliceY)],
                                         width=1,
                                         fill='blue',
                                         dash='-')
    VertPlotCanvas1.tag_bind(lineID, "<Button-1>", horizClickMethod)
    lineID = VertPlotCanvas2.create_line([(0, sliceY), (plotD, sliceY)],
                                         width=1,
                                         fill='blue',
                                         dash='-')
    ...
    #the vertical slice
    lineID = Ezcanvas.create_line([(sliceX, 0), (sliceX, canvasY)],
                                  width=1,
                                  fill='yellow',
                                  dash='-')
    Ezcanvas.tag_bind(lineID, "<Button-1>", vertClickMethod)
    lineID = EzRMScanvas.create_line([(sliceX, 0), (sliceX, canvasY)],
                                     width=1,
                                     fill='green',
                                     dash='-')
    EzRMScanvas.tag_bind(lineID, "<Button-1>", vertClickMethod)
    lineID = HorizPlotCanvas.create_line([(sliceX, 0), (sliceX, plotD)],
                                         width=1,
                                         fill='blue',
                                         dash='-')
    ...
    if n <= nCount:
        EzRMScanvas.create_text(canvasX / 2,
                                canvasY / 2,
                                text=str(nCount - n),
                                fill="Red",
                                font=("Helvetica", "75"))
        ...
        EzRMScanvas.create_text(canvasX / 2,
                                canvasY / 2,
                                text=str(nAvgStable - nAveraging),
                                fill="Magenta",
                                font=("Helvetica", "75"))
  ...
  If fastForwardWithAvg==True and nAveraging<nAvgStable, then we're running to get good averaging"""
    ...
    if n <= nCount or (fastForwardWithAvg and nAveraging < nAvgStable):
        ...
        if fastForwardWithAvg:
            Ezcanvas.create_text(canvasX / 2,
                                 canvasY / 2,
                                 text=str(nAvgStable - nAveraging),
                                 fill="Magenta",
                                 font=("Helvetica", "75"))
...
def step(avg=True):
    ...
    Hx[HXx_range, HXy_range] = DaY * Hx[HXx_range, HXy_range] + DbY * (
        Ez[HXx_range, HXy_range] - Ez[HXx_range, 1:(NHXy + 1)])  #HXy_range+1
    Hy[HYx_range, HYy_range] = DaX * Hy[HYx_range, HYy_range] + DbX * (
        Ez[1:(NHYx + 1), HYy_range] - Ez[HYx_range, HYy_range])  #HYx_range+1

    #do the normal Yee updates on Ez in the relevant range
    Ezx[EZx_range, EZy_range] = CaX * Ezx[EZx_range, EZy_range] + CbX * (
        Hy[EZx_range, EZy_range] - Hy[0:(NEZx - 2), EZy_range])
    Ezy[EZx_range, EZy_range] = CaY * Ezy[EZx_range, EZy_range] + CbY * (
        Hx[EZx_range, 0:NEZy - 2] - Hx[EZx_range, EZy_range])
...
viewmenu.add_radiobutton(label="Lines",
                         variable=traceSetting,
                         value="line",
                         command=conditionalRedraw)
viewmenu.add_radiobutton(label="Dots",
                         variable=traceSetting,
                         value="dot",
                         command=conditionalRedraw)
...
viewmenu.add_radiobutton(label="Amplitude",
                         variable=avgSetting,
                         value="amp",
                         command=conditionalRedraw)
viewmenu.add_radiobutton(label="Ez_rms",
                         variable=avgSetting,
                         value="rms",
                         command=conditionalRedraw)
viewmenu.add_radiobutton(label="Ez_rms^2",
                         variable=avgSetting,
                         value="sq",
                         command=conditionalRedraw)
...
viewmenu.add_checkbutton(label="Gap to slice lines",
                         variable=distLineSetting,
                         onvalue="lines",
                         offvalue="nolines",
                         command=conditionalRedraw)
...
simmenu.add_command(label="Reset Simulation",
                    accelerator="Ctrl+Shift+R",
                    command=reset)
root.bind("<Control-R>", lambda arg: reset())
simmenu.add_command(label="Reset Average",
                    accelerator="Ctrl+Shift+A",
                    command=resetAveraging)
root.bind("<Shift-A>", lambda arg: resetAveraging())
simmenu.add_command(label="Fast Forward",
                    accelerator="Ctrl+F",
                    command=fastForward)
...
tkinter.ttk.Label(viewFrame, textvariable=xStringVar).grid(column=3,
                                                           row=3,
                                                           sticky='nsew',
                                                           padx=5,
                                                           pady=0)
tkinter.ttk.Label(viewFrame, textvariable=yStringVar).grid(column=3,
                                                           row=4,
                                                           sticky='nsew',
                                                           padx=5,
                                                           pady=0)
tkinter.ttk.Label(viewFrame, textvariable=tStringVar).grid(column=3,
                                                           row=5,
                                                           sticky='nsew',
                                                           padx=5,
                                                           pady=0)
tkinter.ttk.Label(viewFrame, textvariable=EzStringVar).grid(column=3,
                                                            row=6,
                                                            sticky='nsew',
                                                            padx=5,
                                                            pady=0)
tkinter.ttk.Label(viewFrame, textvariable=EzRMSStringVar).grid(column=3,
                                                               row=7,
                                                               sticky='nsew',
                                                               padx=5,
                                                               pady=0)
...
tkinter.ttk.Button(barrierFrame, text='Add Opening',
                   command=addOpening).grid(column=0,
                                            row=0,
                                            columnspan=2,
                                            sticky='nsew',
                                            padx=5,
                                            pady=5)
tkinter.ttk.Label(barrierFrame, text="Bottom:").grid(column=0,
                                                     row=1,
                                                     sticky='nes',
                                                     padx=5,
                                                     pady=5)
...
bottomEntry.bind(
    "<Return>",
    lambda arg: root.focus())  #removes focus after the number is entered
bottomEntry.grid(column=1, row=1, sticky='nsw', padx=5, pady=5)
tkinter.ttk.Label(barrierFrame, text="Top:").grid(column=0,
                                                  row=2,
                                                  sticky='nes',
                                                  padx=5,
                                                  pady=5)
